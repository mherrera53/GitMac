...
struct InlineDiffText: View {
    ...
    init(segments: [DiffSegment], side: DiffSide, font: Font = .system(size: 12, design: .monospaced)) {
    ...
    private func textColor(for type: DiffSegment.SegmentType) -> Color {
        ...
        case .removed:
            return Color.red
        case .changed:
            return side == .old ? Color.red : Color.green
        }
    }
    ...
    private func backgroundColor(for type: DiffSegment.SegmentType) -> Color {
        ...
        case .changed:
            return (side == .old ? Color.red : Color.green).opacity(0.25)
        }
    }
}
