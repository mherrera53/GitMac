    ...
    private var toolbarView: some View {
        HStack(spacing: 12) {
            ...
            Picker("View Mode", selection: $viewMode) {
                Label("Inline", systemImage: "list.bullet").tag(DiffViewMode.inline)
...
// DiffViewMode is defined in UI/Components/Diff/DiffToolbar.swift
...
struct UnifiedHunkView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Hunk header
            AdvancedHunkHeaderView(hunk: hunk, isSelected: isSelected)
...
// Uses local AdvancedDiffSide enum defined above

// MARK: - Split Diff Pane

struct SplitDiffPane: View {
    let hunks: [DiffHunk]
    let side: AdvancedDiffSide
    let showWhitespace: Bool
    ...
    var body: some View {
        LazyVStack(alignment: .leading, spacing: 0) {
            ForEach(hunks) { hunk in
                VStack(alignment: .leading, spacing: 0) {
                    // Hunk header
                    AdvancedHunkHeaderView(hunk: hunk, isSelected: selectedHunk?.id == hunk.id)
    ...
    private func shouldShowLine(_ line: DiffLine, for side: AdvancedDiffSide) -> Bool {
